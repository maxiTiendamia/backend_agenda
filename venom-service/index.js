...
async function reconectarSesion(clienteId) {
  ...
  if (reconnectIntervals[clienteId]) {
    console.log(
      `⏳ Ya hay una reconexión en progreso para ${clienteId}, saltando...`
    );
    return;
  }
  ...
  try {
    ...
    if (sessions[clienteId]) {
      try {
        ...
        console.log(
          `⚠️ Error cerrando sesión anterior para ${clienteId}:`,
          e.message
        );
      }
    }

    // Espera obligatoria (10 segundos para asegurar que Chrome cerró correctamente)
    await new Promise((resolve) => setTimeout(resolve, 10000));

    // **NUEVO: Esperar antes de crear nueva sesión para evitar conflictos**
    console.log(
      `⏳ Esperando 5 segundos antes de crear nueva sesión ${clienteId}...`
    );
...
async function monitorearSesiones() {
  console.log(
    `🔍 Monitoreando ${Object.keys(sessions).length} sesiones activas...`
  );
  ...
  for (const clienteId in sessions) {
    ...
    if (reconnectIntervals[clienteId]) {
      console.log(
        `⏳ Saltando monitoreo de ${clienteId} (reconexión en progreso)`
      );
      continue;
    }
    ...
    if (!estaConectada) {
      ...
      if (!reconnectIntervals[clienteId]) {
        if (sessionErrors[clienteId] && sessionErrors[clienteId] >= 3) {
          console.log(
            `🚫 Cliente ${clienteId} bloqueado por errores, saltando reconexión automática`
          );
          continue;
        }
        reconectarSesion(clienteId).catch((err) => {
          console.error(
            `❌ Error en reconexión automática ${clienteId}:`,
            err.message
          );
        });
      }
  ...
  try {
    const client = await venom.create({
      ...
      browserArgs: [
        ...
        "--user-data-dir=" + path.join(SESSION_FOLDER, sessionId),
      ],
      puppeteerOptions: {
        headless: "new",
        args: [
    ...
    client.onStateChange(async (state) => {
      ...
      if (
        ...
        if (reconnectIntervals[sessionId]) {
          console.log(
            `⏳ Reconexión ya en progreso para ${sessionId}, evitando duplicado`
          );
          return;
        }
    ...
    if (
      ...
      console.error(
        `🔒 Error de perfil bloqueado detectado para cliente ${sessionId}. Intentando limpiar SingletonLock y reintentar...`
      );
      await limpiarSingletonLock(sessionId);
...
async function restaurarSesiones() {
  try {
    ...
    try {
      ...
      console.log(
        `📂 Encontradas ${localFolders.length} carpetas válidas en ${SESSION_FOLDER}:`,
        localFolders
      );
    ...
    for (const sessionFolder of sessionFolders) {
      ...
      const sessionPath =
        typeof sessionFolder === "string"
          ? path.join(SESSION_FOLDER, sessionFolder)
          : sessionFolder.path;
      ...
      if (fs.existsSync(defaultPath)) {
        ...
        const tieneLocalStorage = archivosEncontrados.some((a) =>
          a.includes("Local Storage")
        );
        const tienePreferences = archivosEncontrados.some((a) =>
          a.includes("Preferences")
        );
        const tieneIndexedDB = archivosEncontrados.some((a) =>
          a.includes("IndexedDB")
        );
        const tieneSessionStorage = archivosEncontrados.some((a) =>
          a.includes("Session Storage")
        );

        // Criterio MÁS ESTRICTO: requiere Local Storage + (Preferences O IndexedDB)
        tieneArchivosDeSession =
          tieneLocalStorage && (tienePreferences || tieneIndexedDB);

        if (!tieneArchivosDeSession) {
          console.log(`  - ❌ Sesión INCOMPLETA para cliente ${clienteId}:`);
          console.log(
            `    - Local Storage: ${tieneLocalStorage ? "✅" : "❌"}`
          );
          console.log(`    - Preferences: ${tienePreferences ? "✅" : "❌"}`);
          console.log(`    - IndexedDB: ${tieneIndexedDB ? "✅" : "❌"}`);
          console.log(
            `    - Session Storage: ${tieneSessionStorage ? "✅" : "❌"}`
          );
          console.log(
          ...
          console.log(
            `  - ✅ Sesión VÁLIDA para cliente ${clienteId} (Local Storage + archivos adicionales)`
          );
        }
      }
        ...
        try {
          ...
          if (fs.existsSync(defaultPath)) {
            try {
              ...
              console.log(
                `🧹 Carpeta Default eliminada para cliente ${clienteId} antes de restaurar sesión.`
              );
            } catch (err) {
              console.error(
                `❌ Error eliminando carpeta Default para cliente ${clienteId}:`,
                err.message
              );
            }
          }

          // Espera adicional (5 segundos extra después de eliminar carpeta Default)
          await new Promise((resolve) => setTimeout(resolve, 5000));
...
async function limpiarSingletonLock(sessionId) {
  ...
  for (const dir of sessionDirs) {
    const singletonLockPath = path.join(dir, "SingletonLock"); // ✅ Corrección crítica
    if (fs.existsSync(singletonLockPath)) {
      try {
        fs.unlinkSync(singletonLockPath);
        console.log(
          `🔓 SingletonLock eliminado en ${singletonLockPath} para cliente ${sessionId}`
        );
      } catch (err) {
        console.error(
          `❌ Error eliminando SingletonLock en ${singletonLockPath} para ${sessionId}:`,
          err.message
        );
      }
    }
...
app.get("/limpiar/:clienteId", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ok: true,
      mensaje: `SingletonLock limpiado para cliente ${clienteId}`,
    });
  } catch (err) {
    ...
    const estados = await Promise.all(
      clientes.map(async (id) => {
        ...
        if (sessions[id] && sessions[id].isConnected) {
          try {
            estado = (await sessions[id].isConnected())
              ? "conectada"
              : "desconectada";
          } catch {
            estado = "error";
...
app.post("/reiniciar/:clienteId", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ok: true,
      mensaje: `Sesión reiniciada y QR regenerado para cliente ${clienteId}`,
    });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.post("/reset-errores/:clienteId", async (req, res) => {
  ...
  res.json({
    ok: true,
    mensaje: `Errores reseteados para cliente ${clienteId}`,
  });
});
